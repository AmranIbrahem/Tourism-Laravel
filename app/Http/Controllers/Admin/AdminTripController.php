...
class AdminTripController extends Controller
{
    ...
    public function addTrip(addTripRequest $request)
    {
        ...
        if (!$user || $user->Role !== 'admin') {
            return Response::Message("Only admins can add trips!", 403);
        }
    ...
    public function updateTrip(updateTripRequest $request, $trip_id)
    {
        $user = Auth::user();
        if (!$user || $user->Role !== 'admin') {
            return Response::Message("Only admins can update trips!", 403);
        }
    ...
    public function deleteTrip($trip_id)
    {
        $user = Auth::user();
        if (!$user || $user->Role !== 'admin') {
            return Response::Message("Only admins can delete trips!", 403);
        }

        $trip = Trips::find($trip_id);
        if (!$trip) {
            return Response::Message("Trip not found!", 400);
        }
        ...
        if ($trip->delete()) {
            return Response::Message("Trip deleted successfully!", 200);
        } else {
